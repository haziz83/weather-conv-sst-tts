  ...
  function micON() { // When the microphone button is clicked
    if (recording === false) {
      if (records === 0) { // The first time the mic is clicked - inform user
        Api.setRequestPayload({output: {text: ['Accept the microphone prompt in your browser. Watson will listen soon.'], ref: 'STT'}}); // Dialog box output to let the user know we're recording
        records++;
      } else {
        Api.setRequestPayload({output: {ref: 'STT'}}); // Let the user record right away
      }
    } else {
      recording = false;
      stream.stop();
        ...
        stream.promise()                                // Once all data has been processed...
          .then(function(data) {                        // ...put all of it into a single array
            ...
            if (data.length !== 0) {                    // If data is not empty (the user said something)
              ...
              Api.setRequestPayload({output: {text: ['Microphone input cancelled. Please press the button to speak to Watson again']}}); // If the user clicked the microphone button again to cancel current input
            }
          })
